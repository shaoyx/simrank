...
class SRGS : public SimRankMethod{
	public:
		SRGS(int maxsteps, int samplenum, double decayfactor, int samplequerynum,
				int* graph_src, int* graph_dst, int mvid, int udisk, char* sgpath, bool isinit=false) :
			...
			if(uDisk == 1){
				...
				gsm = new RGSManager(samplenum, mvid);
			}
		}
