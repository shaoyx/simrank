...
//constructing output path
	if(strcmp(method, "topsim")==0)
		sprintf(outputpath, "dataset/%s/output/%s%d",inpath, method,tsm_type);
	else if(strcmp(method, "kromsim")==0)
		sprintf(outputpath, "dataset/%s/output/%s%d", inpath,method,kronsim_type);
	else if(strcmp(method,"effisim")==0)
		sprintf(outputpath, "dataset/%s/output/%s%d", inpath,method,effisim_type);
	else if(strcmp(method,"srgs")==0)
		sprintf(outputpath, "dataset/%s/output/%s%d", inpath,method,usDisk);
	else
...
void help() {
    ...
    printf("\t-ts , specify the type of TopSim method [1,2,3].\n");
    ...
    printf("\t-es  <int>, specify the type of Efficient Simrank. [1=EVD,2=SVD,3=optimized SVD]\n");
    printf("\t-r  <int>, specify the rank of matrix-based Simrank.[20]\n");
    printf("\t-psrt <int>, specify the type of PSRSimRank. [0=PAR_SR, 1 =PrunPar_SR]\n");	
}
...
bool pro_args(int argc, char** args) {
	...
	else if(strcmp(args[i], "-es") == 0 && i + 1 < argc){
                ++i;
                effisim_type=atoi(args[i]);
        }
	else if(strcmp(args[i], "-ks") == 0 && i + 1 < argc){
                ++i;
                kronsim_type=atoi( args[i]);
        }
	else if(strcmp(args[i], "-r") == 0 && i + 1 < argc){
                ++i;
                Rank=atoi( args[i]);
        }
	else if(strcmp(args[i], "-psrt")==0 && i+1 < argc){
		++i;
		psr_type=atoi(args[i]);
	}
        else {
