...
int* orig_graph_dst;
//int verticesNum = 456, edgeNum = 71959;
...
int main(int argc, char** argv){
	...
	#ifdef DEBUG
    		printf("InputGraph=%s;\n method=%s;\n num_iter=%d;\ndecay_factor=%.4lf\nsampleNum=%d;\nsampleQueryNum=%d;\nis_fm=%s;\nuDisk=%d;\nhasIndex=%d;\nbuildIndex=%d;\nneedOrig=%d;\ntopsimtype=%d;\nkronsimtype=%d;\neffisimtype=%d;\n rank=%d \n\n", inpath,method, numIter, decayFactor, sampleNum, sampleQueryNum, (isFm == 0 ? "true" : "false"), usDisk, hasIndex, buildIndex, needOrig, tsm_type, kronsim_type, effisim_type,Rank);
	#endif
...
bool getOutputpath(){
    ...
    if(strcmp(method, "topsim")==0){
        ...
        switch(kronsim_type){
            ...
            default: printf("Not supported type %d of kronsim. [valid ones: 0, 1.]\n", kronsim_type);
        }
        sprintf(outputpath, "dataset/%s/output/%s", inpath,method_name);
    }
        ...
        switch(effisim_type){
            ...
            default: printf("Not supported type %d of effisim. [valid ones: 0, 1, 2.]\n", effisim_type);
        }
        sprintf(outputpath, "dataset/%s/output/%s", inpath,method_name);
    }
        ...
        switch(usDisk){
            ...
            default:printf("Not supported type %d of srgs. [valid ones: 0, 1, 2, 3.]\n", usDisk);
        }
        sprintf(outputpath, "dataset/%s/output/%s", inpath,method_name);
    }
        ...
        switch(psr_type){
            ...
            default:printf("Not supported type %d of PSR. [valid ones: 0, 1.]\n", psr_type);
        }
        sprintf(outputpath, "dataset/%s/output/%s", inpath,method_name);
    }
    else
...
SimRankMethod* createSimRankMethod() {
	...
	if(strcmp(method, MOD14) == 0){
		...
		srm = new SRGS(numIter, sampleNum, decayFactor, sampleQueryNum, graph_src, graph_dst, MAX_VERTEX_NUM, usDisk, sgpath, hasIndex);
	}
	else if(strcmp(method,CuipingLi) == 0){
		if(kronsim_type==1)
		srm = new KronSimRank(decayFactor,orig_graph_src,orig_graph_dst,Rank,MAX_VERTEX_NUM,hasIndex,inpath);
		else if(kronsim_type==0)
		srm = new origKronSimRank(decayFactor,orig_graph_src,orig_graph_dst,Rank,MAX_VERTEX_NUM,hasIndex,inpath);
		else
		printf("Invalid kronSimRank types,valid ones are [0,1]\n");	
	}
	else if(strcmp(method, Yasuhiro)==0){
		if(effisim_type==0)
		srm=new EffiEVD(decayFactor,orig_graph_src,orig_graph_dst,Rank,MAX_VERTEX_NUM,hasIndex,inpath);
		else if(effisim_type==1)
		srm=new EffiSVD(decayFactor,orig_graph_src,orig_graph_dst,Rank,MAX_VERTEX_NUM,hasIndex,inpath);
		else if(effisim_type==2)
                srm=new OptEffiSVD(decayFactor,orig_graph_src,orig_graph_dst,Rank,MAX_VERTEX_NUM,hasIndex,inpath);
		else{
		printf("valid type is [0=EffiEVD,1=EffiSVD,2=OptEffiSVD]\n");
		}
	}
	else if(strcmp(method,YuOIP)==0){
		srm = new OIPSimRank(numIter,decayFactor,graph_src,graph_dst,MAX_VERTEX_NUM, inpath, hasIndex);
	}
	else if(strcmp(method,YuVLDB15)==0){
		srm = new PSRSimRank(numIter,decayFactor,graph_src,graph_dst,orig_graph_src,orig_graph_dst,MAX_VERTEX_NUM,psr_type);
	}
	else{
...
void do_computation(int qv, int k, SimRankMethod* srm){
    srm->run(qv, k);
    printf("after running query(qv,k) \n");
...
void help() {
    ...
    printf("\t-ud\t<int>, specify using disk to store the sampled graph [0=original, 1=disk, 2=compress],3=Dynamic.\n");
